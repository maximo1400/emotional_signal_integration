...
def convert_mat_to_df(mat_path: str, io_path: str = None) -> pd.DataFrame:
    """
    Reads DREAMER.mat and converts it to a DataFrame without using torcheeg.
    io_path is kept for API compatibility but ignored.
    """
    mat_data = scio.loadmat(mat_path, verify_compressed_data_integrity=False)
    
    subject_len = len(mat_data['DREAMER'][0, 0]['Data'][0])
    trial_len = len(mat_data['DREAMER'][0, 0]['Data'][0, 0]['EEG'][0, 0]['stimuli'][0, 0])
    ...
    for subject in range(subject_len):
        for trial_id in range(trial_len):
            ...
            dominance = mat_data['DREAMER'][0, 0]['Data'][0, subject]['ScoreDominance'][0, 0][trial_id, 0]
